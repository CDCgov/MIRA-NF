...
usage() {
	echo -e "Usage in git cloned CLI: \n bash $0 -i {path to samplesheet.csv } -o <run_id > -e <experiment_type> -f <nextflow_profiles> <OPTIONAL: -p amplicon_library> <optional: -q processing_q> <optional: -m email_address> <optional: -n > " 1>&2
	exit 1
}
...
while getopts 'i:o:e:p:f:q:m:na' OPTION; do
	case "$OPTION" in
	i) INPUT="$OPTARG" ;;
	o) RUNPATH="$OPTARG" ;;
...
nextflow run $SCRIPTSDIR/main.nf \
	--input "$INPUT" \
	--outdir "$RUNPATH" \
	--e "$EXPERIMENT_TYPE" \
	--p "$PRIMER_SCHEMA" \
